...
function Conversation({ thread, adminId, onNewMessage, onBack }) {
  ...
  const send = async () => {
    ...
    try {
      ...
      const res = await api.post(`/messages/admin/reply/${thread.memberId}`, fd, { headers: { 'Content-Type': 'multipart/form-data' } });
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 pb-3 mb-3 border-b border-gray-100 flex-shrink-0">
        {onBack && (
          <button onClick={onBack} className="text-gray-400 hover:text-green-700 mr-1 text-lg leading-none">←</button>
        )}
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-gray-400 truncate">{roleLabel[thread.member?.role]} · {thread.member?.email}</p>
        </div>
        <button onClick={() => fetchMessages()} className="ml-auto text-gray-400 hover:text-green-700 text-xs flex-shrink-0" title="Actualiser">🔄</button>
      </div>

      {/* Messages */}
      <div className="flex-1 overflow-y-auto space-y-3 min-h-0 pb-2">
        {loading ? (
          <div className="flex justify-center py-10"><div className="w-6 h-6 border-2 border-green-700 border-t-transparent rounded-full animate-spin" /></div>
        ) : error ? (
          ...
          <div key={msg.id} className={`flex ${isAdmin(msg) ? 'justify-end' : 'justify-start'}`}>
            <div className="max-w-[80%]">
              {!isAdmin(msg) && <p className="text-xs text-gray-400 mb-1 ml-1">{thread.member?.firstName}</p>}
              <div className={`rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${isAdmin(msg) ? 'bg-green-800 text-white rounded-tr-sm' : 'bg-gray-100 text-gray-800 rounded-tl-sm'}`}>
                ...
                {msg.fileName && (
                  <a href={msg.viewUrl || '#'} target="_blank" rel="noopener noreferrer"
                    ...
                    <span>📎</span><span className="truncate max-w-[160px]">{msg.fileName}</span><span>↗</span>
                  </a>
                )}
              </div>
      ...
      <div className="border-t border-gray-100 pt-3 flex gap-2 items-end flex-shrink-0">
        <div>
          <input ref={fileInputRef} type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" id={`adm-f-${thread.memberId}`} onChange={e => setFile(e.target.files?.[0] || null)} />
          <label htmlFor={`adm-f-${thread.memberId}`} className="flex items-center justify-center w-9 h-9 rounded-xl border border-gray-200 text-gray-400 hover:border-green-400 hover:text-green-700 cursor-pointer transition-colors">📎</label>
        </div>
...
function BroadcastPanel() {
  ...
  const fetchBroadcasts = useCallback(async () => {
    try { const res = await api.get('/messages/admin/broadcasts'); setBroadcasts(res.data.broadcasts || []); }
    catch { /* silencieux */ }
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-3">
          {Object.entries(AUDIENCE_LABELS).map(([val, label]) => (
            <button key={val} onClick={() => setAudience(val)}
              className={`px-3 py-2 rounded-xl text-xs font-medium border transition-all ${audience === val ? 'bg-green-800 text-white border-green-800' : 'bg-white text-gray-600 border-gray-200 hover:border-green-400'}`}>
        ...
        <div className="flex items-center gap-2">
          <div>
            ...
            <label htmlFor="bc-file" className="flex items-center justify-center w-9 h-9 rounded-xl border border-gray-200 text-gray-400 hover:border-green-400 hover:text-green-700 cursor-pointer transition-colors text-base">📎</label>
          </div>
...
export default function AdminMessagerie() {
  ...
  const fetchThreads = useCallback(async () => {
    try { const res = await api.get('/messages/admin/threads'); setThreads(res.data.threads || []); }
    catch { /* silencieux */ }
    ...
    <MemberLayout title="Messagerie">
      ...
      <div className="flex gap-2 mb-4">
        <button onClick={() => setTab('prive')}
          ...
          ✉️ Messages privés
          {threads.length > 0 && (
            <span className={`ml-1.5 text-xs px-1.5 py-0.5 rounded-full ${tab === 'prive' ? 'bg-white/20 text-white' : 'bg-green-100 text-green-800'}`}>
              {threads.length}
            </span>
          )}
        </button>
      ...
      {tab === 'prive' ? (
        /* ── Layout Messages Privés ── */
        <div className="flex gap-4" style={{ minHeight: '560px' }}>

          {/* Liste threads — cachée sur mobile si une conv est ouverte */}
          <div className={`flex-shrink-0 card p-0 overflow-hidden flex flex-col
            ...
            <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
              ...
              <button onClick={fetchThreads} className="text-gray-400 hover:text-green-700 text-xs">🔄</button>
            </div>
          ...
          {/* Conversation — plein écran sur mobile */}
